...
impl Header {
    fn can_provide(&self, size: usize, align: usize) -> bool {
        ...
        // HEADER_SIZE * 2 => one for allocated region, anoter for padding.
    ...
    //
    // Note: std::aclloc::Layout doc says:
...
pub struct FierstFitAllocator {
    first_header: RefCell<Option<Box<Header>>>,
}

#[global_allocator]
pub static ALLOCATOR: FierstFitAllocator = FierstFitAllocator {
    first_header: RefCell::new(None),
};

unsafe impl Sync for FierstFitAllocator {}

unsafe impl GlobalAlloc for FierstFitAllocator {
...
impl FierstFitAllocator {

    /* NG実装例
    pub fn alloc_with_options(&self, _layout: Layout) -> *mut u8 {
        0x100000 as *mut u8 // どんなレイアウトでも同じアドレスを返す
    }
    */
    ...
    fn add_free_from_descriptor(&self, desc: &EfiMemoryDescriptor) {
        ...
        // It's okay to be sorted the headers at this point
...
mod test {
    ...
    fn malloc_iterate_free_and_alloc() {
        ...
        for i in 0..1000 {
            let mut vec = Vec::new();
            vec.resize(i, 10);
            // vec will be deallocated at the end of the scope
        }
    }
