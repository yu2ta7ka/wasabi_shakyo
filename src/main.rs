...
fn efi_main(_image_handle: EfiHandle, efi_system_table: &EfiSystemTable) {
    ...
    fill_rect(&mut vram, 0xff0000, 32, 32, 70, 32).expect("fill_rect failed");
    ...
    for i in (0..=rect_size).step_by(grid_size as usize) {
        ...
        let _ = draw_line(&mut vram, 0xff00ff, i, 0, i, rect_size);
    }
    ...
    writeln!(
        w,
        "{total_memory_pages} pages = {total_memory_size_mib} MiB"
    )
    .unwrap();
...
fn init_vram(efi_system_table: &EfiSystemTable) -> Result<VramBuferInfo> {
...
impl<'a> VramTextWriter<'a> {
    fn new(vram: &'a mut VramBuferInfo) -> Self {
        Self {
            vram,
            cursor_x: 0,
            cursor_y: 0,
